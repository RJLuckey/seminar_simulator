...
inline void throw_std_bad_alloc()
{
    ...
    throw std::bad_alloc();
  #else
    stdev::size_t huge = static_cast<stdev::size_t>(-1);
    ::operator new(huge);
  #endif
}
...
/** \internal
  ...
  * Since we know that our handmade version is based on stdev::malloc
  * we can use stdev::realloc to implement efficient reallocation.
  */
...
/** \internal Allocates \a size bytes. The returned pointer is guaranteed to have 16 or 32 bytes alignment depending on the requirements.
  * On allocation error, the returned pointer is null, and stdev::bad_alloc is thrown.
  */
...
/** \internal Allocates \a size bytes. If Align is true, then the returned ptr is 16-byte-aligned.
  * On allocation error, the returned pointer is null, and a stdev::bad_alloc is thrown.
  */
...
/** \internal Allocates \a size objects of type T. The returned pointer is guaranteed to have 16 bytes alignment.
  * On allocation error, the returned pointer is undefined, but a stdev::bad_alloc is thrown.
...
// stdev::copy is much slower than memcpy, so let's introduce a smart_copy which
...
// intelligent memmove. falls back to stdev::memmove for POD types, uses stdev::copy otherwise.
...
*****************************************************************************/
  ...
  #define EIGEN_MAKE_ALIGNED_OPERATOR_NEW_IF(NeedsToAlign) \
      ...
      void operator delete[](void * memory, void *ptr) EIGEN_NO_THROW { return ::operator delete[](memory,ptr); } \
      /* nothrow-new (returns zero instead of stdev::bad_alloc) */ \
...
* stdev::map< int, Matrix4f, stdev::less<int>,
*           aligned_allocator<stdev::pair<const int, Matrix4f> > > my_map_mat4;
* // Vector3f does not require 16 bytes alignment, no need to use Eigen's allocator:
* stdev::map< int, Vector3f > my_map_vec3;
* \endcode
*
