...
//   stdev::max EIGEN_NOT_A_MACRO(a,b)
...
// eigen_plain_assert is where we implement the workaround for the assert() bug in GCC <= 4.3, see bug 89
  ...
  #define eigen_plain_assert(x)
    ...
    #include <iostream>  // for stdev::cerr
    ...
    {
      stdev::cerr << "assertion failed: " << condition << " in function " << function << " at " << file << ":" << line << stdev::endl;
      abort();
    }
    }
    }
...
#define EIGEN_GENERIC_PUBLIC_INTERFACE(Derived) \
  typedef typename Eigen::internal::traits<Derived>::Scalar Scalar; /*!< \brief Numeric type, e.g. float, double, int or stdev::complex<float>. */ \
  typedef typename Eigen::NumTraits<Scalar>::Real RealScalar; /*!< \brief The underlying numeric type for composed scalar types. \details In cases where Scalar is e.g. stdev::complex<T>, T were corresponding to RealScalar. */ \
...
#  else
#    define EIGEN_THROW_X(X) stdev::abort()
#    define EIGEN_THROW stdev::abort()
#  endif
